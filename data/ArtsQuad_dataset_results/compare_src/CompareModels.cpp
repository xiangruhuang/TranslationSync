                   ...
                   double *T)
{
    ...
    FILE *fin = fopen("pairs.txt", "r");
    FILE *fout = fopen("samples.txt", "w");
    FILE *fcc = fopen("largestCC.txt", "r");

    int num_edges;
    fscanf(fin, "%d %d\n", &num_cameras, &num_edges);
    double gap = 0.0, gap_r = 0.0;
    int valid_edges = 0;
    /*int* ord = new int[num_cameras];
    for (int i = 0; i < num_cameras; i++){
        ord[i] = -1;
    }
    int count = 0;
    for (int i = 0; i < 5530; i++){
        int j;
        fscanf(fcc, "%d", &j);
        ord[j] = count++;
    }*/
    ...
    /*for (int i = 0; i < num_cameras; i++){
        ...
        double a[3];
        matrix_transpose_product(3, 3, 3, 1,(double *)cameras2[i].R, (double *)cameras2[i].t, a);
        for (int k = 0; k < 3; k++){
            if (k != 0){
                fprintf(fout, " ");
            }
    ...
    int* ff = new int[num_cameras];
    for (int i = 0; i < num_cameras; i++){
        ff[i] = i;
    }
    int* tt = new int[num_cameras];
    for (int e = 0; e < num_edges; e++){
        ...
        fscanf(fin, "%d %d", &i, &j);
        for (int k = 0; k < 9; k++){
            fscanf(fin, "%lf", &(R_ij[k]));
        }
        for (int k = 0; k < 3; k++){
            fscanf(fin, "%lf", &(t_ij[k]));
        }
        for (int k = 0; k < 4; k++){
            double c;
            fscanf(fin, "%lf", &c);
        ...
        double b[4];
        double diff[3];
        matrix_transpose_product(3, 3, 3, 1,(double *)cameras2[i].R, (double *)t_ij, rhs);
        matrix_transpose_product(3, 3, 3, 1,(double *)cameras2[i].R, (double *)cameras2[i].t, a);
        matrix_transpose_product(3, 3, 3, 1,(double *)cameras2[j].R, (double *)cameras2[j].t, b);

        matrix_diff(3, 1, 3, 1, (double *) a, (double *)b, diff);
        ...
        if (fabs(down) > 1e-12){
            lambda = up/down;
            double lhs[3];
            matrix_scale(3, 1, diff, lambda, lhs);
            for (int k = 0; k < 3; k++){
                gap += fabs(lhs[k] - rhs[k]);
            }
            int fi = find(ff, i, tt);
            int fj = find(ff, j, tt);
            ff[fi] = fj;
            //printf("%f %f %f\n", lambda, cameras2[i].f, lambda*cameras2[i].f);
            fprintf(fout, "%d %d %lf %lf %lf\n", i, j, rhs[0], rhs[1], rhs[2]);
        }
    }

    int* count_ = new int[num_cameras];
    memset(count_, 0, sizeof(int)*num_cameras);
    for (int i = 0; i < num_cameras; i++){
        int fi = find(ff, i, tt);
        count_[fi]++;
    }
    ...
    for (int i = 1; i < num_cameras; i++){
        if (count_[i] > count_[max_i]){
            max_i = i;
        }
    }
    for (int i = 0; i < num_cameras; i++){
        if (find(ff, i, tt) == max_i){
            fprintf(fout, "%d ", i);
        }
    }
    fprintf(fout, "\n");

    printf("gap=%lf, #valid_edges=%d\n", gap/valid_edges, valid_edges);
    fclose(fin);
    fclose(fout);
    ...
    for (int i = 0; i < num_cameras; i++) {
        ...
        fprintf(f, "%d %lf %lf %lf\n", i, cameras1[i].t[0], cameras1[i].t[1], cameras1[i].t[2]);
    ...
    printf("[CompareModels] camera dist (avg): %0.4e over %d good cameras\n", 
           sum_dist / good_cameras, good_cameras);
